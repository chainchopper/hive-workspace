...
All notable changes to THE HIVE are documented here.
...
**Zero-fork release.** Clone, don't fork. THE HIVE now runs on vanilla `pip install hermes-agent` with no patches, no drift, no custom gateway required.
